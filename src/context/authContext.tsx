...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const refreshUserData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/auth/users/${user.login}`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (rolesResponse.ok) {
          ...
          if (userRole) {
            ...
            // Obtener los permisos del rol
            const permissionsResponse = await fetch(`${API_BASE_URL}/roles/${userRole.id}/permisos`, {
              headers: {
                'Authorization': `Bearer ${token}`
              }
            });
            ...
            if (permissionsResponse.ok) {
              ...
              const permissionsNames = await Promise.all(
                permissionsData.map(async (permId: number) => {
                  const permResponse = await fetch(`${API_BASE_URL}/permisos/${permId}`, {
                    headers: {
                      'Authorization': `Bearer ${token}`
                    }
                  });
